...
function qode_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
		...
		$check = array('anchor', 'nolink', 'hide', 'type_menu', 'icon', 'sidebar', 'wide_position', 'show_widget_area_in_popup');
...
if (!class_exists('qode_type1_walker_nav_menu')) {
	...
	// add classes to ul sub-menus
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ((($item->current && $depth == 0) ||  ($item->current_item_ancestor && $depth == 0)) && ($qode_options_proya['page_transitions'] == "0")):
...
if (!class_exists('qode_type3_walker_nav_menu')) {
    class qode_type3_walker_nav_menu extends Walker_Nav_Menu {
        ...
        function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            // depth dependent classes
            if ((($item->current && $depth == 0) ||  ($item->current_item_ancestor && $depth == 0)) && ($qode_options_theme16['page_transitions'] == "0")):
                $active = 'active';
            endif;
